...
public class BasicEncryptionExample {
	...
	static void encryptAndDecrypt(final String keyArn) {
		...
		// commitment policy,
		// which enforces that this client only encrypts using committing algorithm
		// suites and enforces
		// that this client will only decrypt encrypted messages that were created with
		// a committing algorithm suite.
		...
		// 2. Instantiate an AWS KMS master key provider in strict mode using
		// buildStrict().
		// In strict mode, the AWS KMS master key provider encrypts and decrypts only by
		// using the key
		// indicated by keyArn.
		// To encrypt and decrypt with this master key provider, use an &KMS; key ARN to
		// identify the &KMS; keys.
